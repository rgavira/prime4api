...
class RateEvaluator:
    ...
    def capacity_at(self, t: Union[str, TimeDuration]):
        ...
        value, period = self.rate.value, self.rate.period
    ...
    def min_time(self, capacity_goal: int, return_unit: Optional[TimeUnit] = None, display = True) -> Union[str, TimeDuration]:
        ...
        T = np.floor((capacity_goal - 1) * self.rate.period.to_milliseconds() / self.rate.value) if capacity_goal > 0 else 0
        ...
        if return_unit is None:
            return_unit = self.rate.period.unit
