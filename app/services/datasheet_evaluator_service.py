...
class DatasheetEvaluatorService:
    ...
    def evaluate(self, yaml_data: dict, request: EvaluateDatasheetRequest) -> List[EvaluateDatasheetResultItem]:
        ...
        plan_global_rate = plan_data.get("rate")
        plan_global_quota = plan_data.get("quota")

        endpoints_data = plan_data.get("endpoints", {})
        
        results = []

        # 3. Iterar o ir a endpoint específico
        ...
        # 4. Procesar Endpoints y Alias Anidados
            ...
            if has_aliases:
                ...
                for alias_name, alias_config in aliases_to_process.items():
                    res = self._process_node(
                        ...
                        plan_global_rate=plan_global_rate,
                        plan_global_quota=plan_global_quota,
    ...
    def _process_node(self, node_config: dict, capacity_defs: dict, max_power_defs: dict,
                      plan_global_rate: Optional[str], plan_global_quota: Optional[str],
                      operation: str, operation_params: dict) -> Any:
        
        # Rate Limiting
        rate_key = node_config.get("rate") or plan_global_rate
        rate_obj = None
        if rate_key:
        ...
        # Quotas
        quota_key = node_config.get("quota") or plan_global_quota
        quota_obj = None
        if quota_key:
        ...
        if not rate_obj and not quota_obj:
        ...
        # Montar y pasar los params kwargs
        kwargs = dict(operation_params)
        kwargs["rate"] = rate_obj
        kwargs["quota"] = quota_obj
